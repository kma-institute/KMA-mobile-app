...
class CourseBloc extends Bloc<CourseEvent, CourseState> {
  CourseBloc() : super(InitialCourseState()) {
    ...
    on<GetTokenToCourse>((event, emit) async {
      ...
      try {
        final response =
            await _coursesRepository.getTokenToCourse(event.courseId);
  ...
  Future<CourseState> _fetchCourse(
      CourseEvent event, Emitter emit, int courseId,) async {
    emit(InitialCourseState());
    _countTab = 2;
    try {
      courseDetailResponse = await _coursesRepository.getCourse(courseId);

      if (courseDetailResponse!.faq != null &&
          courseDetailResponse!.faq!.isNotEmpty) {
        _countTab = 2;
      }
